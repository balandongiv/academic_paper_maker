...
For springer, unfortunately, UMS does not have the license to access the full text. However, we can still download the metadata and save the URL for future reference. The code below will download the metadata and save the URL for future reference.
'''
...
def login_springer(driver):
    ...
    """
    urls = ["https://link.springer.com/article/10.1007/s11063-022-10858-x"]
    driver.get(urls[0])
    ...
    with tqdm(total=total_urls, desc="Processing URLs") as pbar:
        for bibtex, urls, title in flattened_entries:
            for url in urls:
                ...
                driver.get("https://link.springer.com/")  # Navigate to a page before adding cookies
                driver.delete_all_cookies()
                for c in cookies:
                    driver.add_cookie(c)
                driver.get("https://link.springer.com/")  # Refresh page to apply cookies
                    ...
                    if '.pdf' in url.lower():
                        ...
                        pdf_link = WebDriverWait(driver, 10).until(
                            EC.element_to_be_clickable(
                                (By.CSS_SELECTOR, "a[data-test='pdf-link']")
                            )
                        )
                        pdf_link.click()
...
def do_download_springer(input_urls):
    """
...
def main():
    ...
    do_download_springer(input_urls)
